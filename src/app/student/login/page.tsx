      ...
      if (loggedInUser && loggedInUser.role === 'Student') {
        ...
        router.push('/student/dashboard');
         ...
         toast({
            variant: 'destructive',
            title: 'Login Failed',
            description: 'Invalid student email. Please try again.',
        });
      }
    } catch (error: any) {
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-10rem)] py-12 px-4">
      <Card className="w-full max-w-md glass-card">
        <CardHeader className="text-center pb-4">
          ...
          <CardDescription>Enter your student email to access your portal.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSignIn} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" placeholder="student@klu.ac.in" required value={email} onChange={e => setEmail(e.target.value)} />
            </div>
            <div className="space-y-2">
           ...
           <p className="text-xs text-muted-foreground mt-4 text-center">
              Hint: Use one of the registered student emails. Any password will work.
            </p>
        </CardContent>
      </Card>
    </div>
